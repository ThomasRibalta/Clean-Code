...
void appendAtStart(char array[MAX_FRUITS][MAX_FRUIT_NAME], int *size, const char *fruit) {
    if (*size >= MAX_FRUITS) {
        printf("Le tableau est plein. Impossible d'ajouter un nouvel élément.\n");
        return;
    }
...
void appendAtEnd(char array[MAX_FRUITS][MAX_FRUIT_NAME], int *size, const char *fruit) {
    if (*size >= MAX_FRUITS) {
        printf("Le tableau est plein. Impossible d'ajouter un nouvel élément.\n");
        return;
    }
...
int main() {
    ...
    appendAtEnd(ArrayFruits, &size, "Banane");
    appendAtEnd(ArrayFruits, &size, "Orange");
    printf("Tableau actuel : ");
    displayArray(ArrayFruits, size);
    appendAtStart(ArrayFruits, &size, "Fraise");
    printf("Tableau après insertion au début : ");
    displayArray(ArrayFruits, size);
    return 0;
}
